...
public class Day18 {
    ...
    public static Coord block(String input) {
        ...
        for (int i = 1024; i < s.length; i++) {
            ...
            int dist = shortest(grid);

            if (dist == 0) {
                Coord block = new Coord(x, y);
    ...
    public static int shortest(int[][] grid) {
        ...
        Map<Coord, Integer> dists = new HashMap<Coord, Integer>();
        ...
        while (!pq.isEmpty()) {
            ...
            ArrayList<Coord> adj = adjacent(grid, curr);
            ...
            for (Coord c : adj) {
                if (valid(grid, c)) {
                    if (w.getDistance() + 1 < dists.getOrDefault(c, Integer.MAX_VALUE)) {
                        ...
                        Pair n = new Pair(c, w.getDistance() + 1);
                        pq.add(n);
                    }
                }
            }
    ...
    public static boolean valid(int[][] grid, Coord c) {
        return c.getX() >= 0 && c.getY() >= 0 && c.getX() < grid.length && c.getY() < grid[0].length && grid[c.getX()][c.getY()] != 1;
    }

    public static ArrayList<Coord> adjacent(int[][] grid, Coord curr) {
        ArrayList<Coord> adjacent = new ArrayList<Coord>();

        Coord up = new Coord(curr.getX() - 1, curr.getY());
        Coord right = new Coord(curr.getX(), curr.getY() + 1);
        Coord down = new Coord(curr.getX() + 1, curr.getY());
        Coord left = new Coord(curr.getX(), curr.getY() - 1);
    ...
    public static class Pair  implements  Comparable<Pair>{
        ...
        public Pair(Coord c, int distance) {
            this.point = c;
